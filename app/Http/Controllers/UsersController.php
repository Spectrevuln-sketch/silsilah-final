...
class UsersController extends Controller
{
    ...
    {
        $request->validate([
            'photo' => 'required|image|file|max:6000',
        ]);
        ...
        $request->file('photo')->storeAs('/', $fileName, 'uploads' );
        // $request->file('photo')->store($user->photo_path);
        
        $user->save();

        // return back()->with('success','You have successfully upload file.')
        // ->with('file',$fileName);
        return back()->with('success','Berhasil Upload Foto')->with('file',$fileName);
    }
...
/** Redirect to Whatsapp Page */
    ...
    {
        return redirect()->away('https://wa.me/'.$user->phone);
    }



    /** form embed video youtube */
    public function widget_youtube(User $user)
    {
        return view('users.widget', compact('user'));
    }

    /** form embed video youtube */
    public function embedpost(Request $request, User $user)
    {
        $request->validate([
            'embed_youtube' => 'required|string',
        ]);
        $embed = EmbedVideo::create([
            'id' => Uuid::uuid4()->toString(),
            'user_id'=> $user->id,
            'embed_video' => $request->embed_youtube,
        ]); 
        
        $user->update([
            'embed_video_id' => $embed->id,
        ]);
        $embed->save();
